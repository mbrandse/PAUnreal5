...
ECheckBoxState SDialogueViewportWidget::GetIsPlayer() const
{
	...
	{
		int32 index = NodeIdsIndexes.FindRef(Dialogue->CurrentNodeId);
		if (Dialogue->Data[index].NodeOwner == EDialogueNodeOwner::DNO_PLAYER) return ECheckBoxState::Checked;
	}
...
FReply SDialogueViewportWidget::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.GetEffectingButton() == EKeys::RightMouseButton)
	{
		...
		// Right click menu when clicked on the background.
		{
			...
			//PA ADDON
			FUIAction AddTextEventNodeAction(FExecuteAction::CreateSP(this, &SDialogueViewportWidget::OnAddEventNodeClicked));
			//END PA ADDON
			...
			//PA ADDON
			MenuBuilder.AddMenuEntry(NSLOCTEXT("PropertyView", "AddEventNode", "Add Event Node"), NSLOCTEXT("PropertyView", "AddNode_ToolTip", "Adds an empty text event node"), FSlateIcon(), AddTextEventNodeAction);
			//END PA ADDON
...
void SDialogueViewportWidget::OnAddEventNodeClicked()
{
	const FScopedTransaction Transaction(LOCTEXT("AddNode", "Node Added"));
	Dialogue->Modify();
