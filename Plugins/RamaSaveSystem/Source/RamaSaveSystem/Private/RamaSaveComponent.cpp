...
void URamaSaveComponent::BeginPlay()
{
	Super::BeginPlay();
	
	ActorStreamingLevel = GetActorStreamingLevelPackageName();
}
...
bool URamaSaveComponent::RamaSave_LoadFromFile(UWorld* World, int32 RamaSaveSystemVersion, const TArray<FString>& LoadActorsWithSaveTags,  FArchive &Ar, URamaSaveComponent*& LoadedComp, bool DontLoadPlayerPawns, FString LoadOnlyStreamingLevel)
{
	...
	Settings->GetRemappedPath(ActorClassFromFile);
	if (Settings->LogLoadedActorClassPath) UE_LOG(RamaSave, Warning, TEXT("RSC Loaded Class Path is %s"), *ActorClassFullPath);
	...
	FString LevelPackageNameFromFile = "Old File Version, Re-save this file to get level streaming info! <3 Rama";
	if(RamaSaveSystemVersion > 3)
	{ 
		//! 4.9 Level Streaming
		Ar << LevelPackageNameFromFile;
	}
	...
	{ 
		//Not Match?
		if(LoadOnlyStreamingLevel != LevelPackageNameFromFile)
		{
	...
	SaveComp->LoadedLevelPackageName = LevelPackageNameFromFile;
...
void URamaSaveComponent::FullyLoaded()
{
	...
	{ 
		RamaSaveEvent_ActorFullyLoaded.Broadcast(this,LoadedLevelPackageName);
	}
...
void URamaSaveComponent::RamaCPP_PreSave()
{
	...
	//Set Game Version, this is saved to disk because all properties of an RMC are saved to disk.
	LoadedGameVersion = Settings->GameVersion;
...
{ 
	...
	LoadedLevelPackageName = ActorStreamingLevel;
	...
	if(RamaSave_VerboseLog)
	{
		...
		if(ActorBP) 
		{
			...
			{ 
				...
				FProperty* Property = FindFProperty<FProperty>( ActorOwner->GetClass(), *ActorVarName );
				if(Property) 
				{ 
					//Ignore Delegates
					//Delegate? 
...
{ 
	...
	for(int64 v = 0; v < TotalProperties; v++)
	{
		...
		FProperty* Property = FindFProperty<FProperty>( ActorOwner->GetClass(), *PropertyNameString );
		if(Property) 
		{ 
			uint8* InstanceValuePtr = Property->ContainerPtrToValuePtr<uint8>(ActorOwner);  //this = object instance that has this property!
		...
		else
		{
			...
			UE_LOG(RamaSave,Warning,TEXT("Property in save file but not found in class, re-save to get rid of this message. Consider remapping old variable name to new name in Project Settings -> Rama Save System. %s %s"), *ActorOwner->GetClass()->GetName(), *PropertyNameString);
		}
	}
}
...
void URamaSaveComponent::SaveSelfAndSubclassVariables(FArchive &Ar)
{
	...
	for (TFieldIterator<FProperty> It(this->GetClass()); It; ++It)
	{
		...
		FProperty* SuperClassProperty = FindFProperty<FProperty>( SuperClass, *PropertyNameString );
...
void URamaSaveComponent::LoadSelfAndSubclassVariables(FArchive &Ar)
{
	...
	for(int64 v = 0; v < TotalProperties; v++)
	{
		...
		FProperty* Property = FindFProperty<FProperty>( this->GetClass(), *PropertyNameString );
		if(Property) 
		{ 
			uint8* InstanceValuePtr = Property->ContainerPtrToValuePtr<uint8>(this);  //this = object instance that has this property!
		...
		else
		{
			...
			if(GetOwner()) UE_LOG(RamaSave,Warning,TEXT("Property in save file but not found in class, re-save to get rid of this message. Consider remapping old variable name to new name in Project Settings -> Rama Save System. %s %s"), *GetOwner()->GetClass()->GetName(), *PropertyNameString);
		}
	}
...
void URamaSaveComponent::SaveSubComponentVariables(AActor* ActorOwner, UWorld* World, FArchive &Ar)
{
	...
	for(int32 v = 0; v < RamaSave_ComponentVarsToSave.Num(); v++)
	{
		...
		for(int32 b = 0; b < Comps.Num(); b++)
		{
			...
			FProperty* Property = FindFProperty<FProperty>( EachComp->GetClass(), *EachToSave );
			if(Property) 
			{ 
				if(RamaSave_LogAllSavedComponentProperties)
				{
	...
	for(int32 b = 0; b < Comps.Num(); b++)
	{
		...
		for (TFieldIterator<FProperty> It(EachComp->GetClass()); It; ++It)
		{
...
void URamaSaveComponent::LoadSubComponentVariables(AActor* ActorOwner, UWorld* World, FArchive &Ar)
{
	...
	{ 
		...
		{ 
			UActorComponent* EachComp = Comps[b];
			FProperty* Property = FindFProperty<FProperty>( EachComp->GetClass(), *PropertyNameString );
			if(Property) 
			{ 
				if(RamaSave_LogAllSavedComponentProperties)
				{
	...
	for(int32 v = 0; v < TotalCompEntries; v++)
	{
		...
		for(int32 b = 0; b < CompPropertiesTotal; b++)
		{
			...
			FProperty* Property = FindFProperty<FProperty>(FoundComponent->GetClass(), *PropertyNameString);
			if (Property)
			{
				if (RamaSave_LogAllSavedComponentProperties)
				{
